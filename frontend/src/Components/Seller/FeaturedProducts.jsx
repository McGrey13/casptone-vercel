...
export default function FeaturedProducts() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch('https://craftconnect-laravel-backend-2.onrender.com/api/products', {
        headers: {
  ...
  const toggleFeatured = async (productId, currentStatus) => {
    try {
      ...
      const response = await fetch(
        `https://craftconnect-laravel-backend-2.onrender.com/api/products/${productId}/toggle-featured`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
